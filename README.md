...
| **Texture analysis** | PNG / JPEG / WebP / **AVIF** / **HEIC** / **KTX2** / **TIFF** / **ASTC** — complete OpenUSD-Spec coverage via magic-bytes detection. Native browser preview for PNG/JPEG/WebP/AVIF/HEIC (Safari); format label for GPU-compressed formats. Dimensions, thumbnails, PBR channel badges (Diffuse / Normal / Roughness / Metallic / Emissive / Occlusion / Opacity / Displacement / Subsurface / Clearcoat). Status: `used` / `unused` / `unknown` *(v0.24 / v0.24.1 / v0.25.4 / v0.25.5)* |
...
- **v0.27.1** *(2026-05-09)* — Landingpage Verify-Sektion DE + EN (ADR-40): `#verify`-Anker zwischen `#ar-quick-look` und `#anwendung`. Pitch-Absatz, 3-Layer-Trust-Architektur-Block, Python-Code-Snippet (5 Zeilen), CTAs auf Inspector + Verify-Strategy. Anker-Link für LinkedIn / Brevo-Newsletter / AOUSD-Talk-Folien. Pure HTML, kein neuer Dep.
- **v0.28.0** *(2026-05-25)* — Two-File-Pattern (ADR-43) + Inspector Advanced mit Three.js Desktop-3D-Preview (ADR-44): `advanced/index.html` (~1.2 MB, inline-Bundle) + `index.html` Standard (200 KB) aus gemeinsamer Quelle `src/inspector.html`. Click-Through Standard ↔ Advanced. 18/18 PASS.
- **v0.28.0.2** *(2026-05-25)* — Graceful Degradation (ADR-45): Lazy-Load Bundle (Advanced: 1244 KB → 217 KB initial), opt-in Start-Button, vollständiger try/catch, Fehler-Toast mit Detail-Klapp. Safari-Freeze bei komplexen Assets unmöglich ohne User-Klick. 18/18 PASS.
- **v0.28.0.3** *(2026-05-25)* — Asset-Klassifikation für Player-Erwartungs-Management (ADR-46): `classifyAssetForPlayer()` analysiert ZIP-Struktur vor dem Render — drei BLOCK-Trigger (Sub-USDZ, ≥5 USDC-Module, Tiny-Coordinator-USDC mit Kompositions-Arcs) + zwei CAUTION-Trigger (>20 MB, 2–4 USDC). Button-Text und -Farbe in drei Zuständen: PASS orange / CAUTION amber / BLOCK grau. Banner zeigt Klassifikations-Grund. USDconfig-Bridge-Block immer sichtbar (DE+EN). Parse-Fail + Blank-Canvas-Toast. 6/6 Real-World-Assets korrekt klassifiziert. Spike-Doku: `docs/v0.28.0.3-spike-results.md`. Kompatibilitäts-Matrix: `docs/PLAYER-ASSET-COMPATIBILITY.md`.
- **v0.28.1** *(2026-06-04)* — Erklärbär: Findings-Klartext (ADR-48). Alle 21 AR_QL_RULES-Findings per Klick aufklappbar — native `<details>`/`<summary>`, Erklärung + Fix-Vorschlag DE+EN, kein LLM, kein API. PDF-Checkbox "mit Erklärungen" (default aus) — Compliance-Audits bleiben knapp, Marketing/Vertrieb bekommt Klartext auf Wunsch. Fix: "Läuft mit Vorbehalt" Umlaut-Bug im PDF. 18/18 PASS.
