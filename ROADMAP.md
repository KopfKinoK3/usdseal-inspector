...
**Aktueller Stand:** v0.25.8 (AR-QL-Regeln Apple-Doku-belegt + 3 Korrekturen + ASTC-Konsistenz-Catch, released 2026-05-06, Commit `2ce334a`). **Sechs Releases in zwei Tagen** v0.25.4 → v0.25.8. Inspector ist jetzt Validator-USP voll ausgereift, gegen Apple-Doku verifiziert, ehrlich kommunizierend (Severity-Recal, USDC-Limitation, AR-QL-Sources).
...
| **v0.25.7** | *USDC-Material-Limitation ehrlich kommunizieren* | Task #56 — UNUSED-Befund aus Frankfurt-Live-Test gelöst: `buildChannelMap` erkennt `usdcBinaryMaterials`, alle unbindigen Texturen → "unknown" statt "unused". Hinweis-Box im UI + PDF-Report. Bugfix: `t` als Loop-Var → `tex` umbenannt (i18n-Function-Shadow). 18/18 PASS. ADR-33. | ✅ released 2026-05-06, Tag online (Commit `627f86b`) |
| **v0.25.8** | *AR-QL-Regeln gegen Apple-Doku korrigiert* | TEXTURE_FORMAT_UNSUPPORTED error→warn, WebP raus, AVIF/HEIC in allowed-list, ASTC-Konsistenz-Catch (fehlte in bad-list — Code-Chat hat es beim Refactor gefangen). STRUCTURE_FILE_SIZE_LIMIT als "viSales-Empfehlung". TEXTURE_TOO_LARGE zweistufig: WARN >2048 + ERROR >4096. AR-QL-RULES.md mit Disclaimer + Sources-Verlinkung. 18/18 PASS. ADR-34. | ✅ released 2026-05-06, Tag online (Commit `2ce334a`) |
